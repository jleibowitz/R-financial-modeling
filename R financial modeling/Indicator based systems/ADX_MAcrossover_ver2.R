...
GBPUSDst<-read.csv(file="C://TickDownloader/tickdata/GBPUSD_M30_UTC+0_00.csv",header=TRUE,stringsAsFactors=FALSE)
...
"sigCrossover" <- function(label,data=mktdata, columns, relationship=c("gt","lt","eq","gte","lte"), offset1=0, offset2=0) {
  ...
  ret_sig<-merge(xx,ret_sig)
  return(ret_sig[,NCOL(ret_sig)])
}
