...
my.obj.func <- function(x)
{
  ...
  #return(max(x$user.func$GBPUSD.Profit.To.Max.Draw) == x$user.func$GBPUSD.Profit.To.Max.Draw)
}

### walk.forward

r <- walk.forward(strategy.st, paramset.label='EMA', portfolio.st=portfolio.st, account.st=account.st, period='months', k.training=3, k.testing=1, nsamples=0, audit.prefix='wfa', anchored=FALSE, verbose=TRUE)
