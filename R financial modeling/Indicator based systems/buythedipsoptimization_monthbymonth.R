timevector<-c("2004-01-01/2012-12-31","2004-01-01/2013-12-31","2004-01-01/2014-12-31","2004-01-01/2015-12-31")
PAIR<-c("GBPUSD","AUDUSD","EURUSD","USDCAD","NZDUSD","USDCHF")
Z<-matrix(nrow=6,ncol=8)
t1=Sys.time()
for(v in 1:length(PAIR)){
...
GBPUSDlt<-read.csv(file=paste0("C://TickDownloader/tickdata/",PAIR[v],"_D1_UTC+0_00.csv"),header=TRUE,stringsAsFactors=FALSE)
...
GBPUSDhr<-read.csv(file=paste0("C://TickDownloader/tickdata/",PAIR[v],"_H1_UTC+0_00.csv"),header=TRUE,stringsAsFactors=FALSE)
...
GBPUSDhr<-(GBPUSDhr[timevector[i]])
GBPUSDmt<-read.csv(file=paste0("C://TickDownloader/tickdata/",PAIR[v],"_M15_UTC+0_00.csv"),header=TRUE,stringsAsFactors=FALSE)
...
GBPUSD<-read.csv(file=paste0("C://TickDownloader/tickdata/",PAIR[v],"_M1_UTC+0_00.csv"),header=TRUE,stringsAsFactors=FALSE)
...
Z[v,i]<-tSA[which(tSA$Net.Trading.PL==max(tSA$Net.Trading.PL)),1]
...
Z[v,(i+4)]<-tSB[which(tSB$Net.Trading.PL==max(tSB$Net.Trading.PL)),1]
}
}
t2=Sys.time()
td=t2-t1
